...
运行前会显示材料模式、发送方式、材料字符数、prompt 字符数、CJK 字符数、每个文本模型 pass 的近似输入 token 数、跨文本模型的近似总输入 token，以及文本模型调用次数。token 预览是粗略的中英混合启发式估算，不是 provider 计费 tokenizer；真实 provider token、输出 token 和视觉结果追加文本仍可能不同。三个及以上文本模型会给出非阻塞预算提醒，不会要求二次确认。
...
审查意见应优先采纳有证据链的问题：文件/行号、原文片段、命令输出、视觉观察或可复现实验。只有结论没有证据的意见，先作为线索而不是事实。
